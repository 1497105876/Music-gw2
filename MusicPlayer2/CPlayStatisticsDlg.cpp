...
CPlayStatisticsDlg::CPlayStatisticsDlg(CWnd* pParent /*=nullptr*/)
    : CBaseDialog(IDD_PLAY_STATISTICS_DLG, pParent)
{
...
void CPlayStatisticsDlg::ShowData(bool size_changed)
{
	//将vector中的数据显示到列表中
    if(size_changed)
...
CPlayStatisticsDlg::ListItem CPlayStatisticsDlg::SongInfoToListItem(const SongInfo& song)
{
    ListItem list_item;
...
BEGIN_MESSAGE_MAP(CPlayStatisticsDlg, CBaseDialog)
	ON_BN_CLICKED(IDC_EXPORT_BUTTON, &CPlayStatisticsDlg::OnBnClickedExportButton)
	ON_WM_GETMINMAXINFO()
    ON_BN_CLICKED(IDC_CLEAR_BUTTON, &CPlayStatisticsDlg::OnBnClickedClearButton)
    ON_NOTIFY(HDN_ITEMCLICK, 0, &CPlayStatisticsDlg::OnHdnItemclickList1)
END_MESSAGE_MAP()
...
void CPlayStatisticsDlg::OnBnClickedExportButton()
{
	// TODO: 在此添加控件通知处理程序代码
...
void CPlayStatisticsDlg::OnBnClickedClearButton()
{
    // TODO: 在此添加控件通知处理程序代码
...
void CPlayStatisticsDlg::OnHdnItemclickList1(NMHDR *pNMHDR, LRESULT *pResult)
{
