...
BOOL CStatArtistRankTabDlg::OnInitDialog()
{
    ...
    m_list.InsertColumn(COL_RANK, L"#", LVCFMT_LEFT, width_rank);       // 排名列
    m_list.InsertColumn(COL_NAME, L"歌手", LVCFMT_LEFT, width_name);    // 歌手名列
    m_list.InsertColumn(COL_VALUE, L"播放时长", LVCFMT_RIGHT, width_value); // 时长列
